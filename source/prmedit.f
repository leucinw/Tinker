...
c
 ...
 1130       format (36x,3f11.5)
            ...
            read (string,*,err=1140,end=1140)  ia,pol,thl,dird,
     &                                         (ig(j),j=1,20)
 1140       continue
            kg = 0
            do j = 1, maxval
            ...
            call sort (kg,ig)
            write (iprm,1150)  ia,pol,thl,dird,(ig(j),j=1,kg)
 1150       format ('polarize',2x,i5,5x,3f11.4,2x,20i5)
...
c
  ...
  410          format ('multipole',1x,4i5,a)
            ...
            string = record(next:240)
            read (string,*,err=420,end=420)  ia,pol,thl,dird,
     &                                       (ig(j),j=1,20)
  420       continue
            ia = itype(ia)
            do j = 1, maxval
            ...
            call sort (kg,ig)
            write (iprm,430)  ia,pol,thl,dird,(ig(j),j=1,kg)
  430       format ('polarize',2x,i5,5x,3f11.4,2x,20i5)
         else if (keyword(1:7) .eq. 'PIATOM ') then
            ia = 0
            call getnumb (record,ia,next)
            ia = iclass(ia)
