...
c
     ...
     &                   - txzk*uix - tyzk*uiy - tzzk*uiz
               ...
               frcx = 2.0d0*pscale(kk)*depx
               frcy = 2.0d0*pscale(kk)*depy
               frcz = 2.0d0*pscale(kk)*depz
c
...
c
     ...
     &                            - urkm*term3
                        depx = txxi*uopt(m,1,k) + txxk*uopt(j,1,i)
     &                       + txyi*uopt(m,2,k) + txyk*uopt(j,2,i)
     &                       + txzi*uopt(m,3,k) + txzk*uopt(j,3,i)
                        depy = txyi*uopt(m,1,k) + txyk*uopt(j,1,i)
     &                       + tyyi*uopt(m,2,k) + tyyk*uopt(j,2,i)
     &                       + tyzi*uopt(m,3,k) + tyzk*uopt(j,3,i)
                        depz = txzi*uopt(m,1,k) + txzk*uopt(j,1,i)
     &                       + tyzi*uopt(m,2,k) + tyzk*uopt(j,2,i)
     &                       + tzzi*uopt(m,3,k) + tzzk*uopt(j,3,i)
...
c
               txi3 = psr3*ukx + psr3*ukx
               tyi3 = psr3*uky + psr3*uky
               tzi3 = psr3*ukz + psr3*ukz
               txk3 = psr3*uix + psr3*uix
               tyk3 = psr3*uiy + psr3*uiy
               tzk3 = psr3*uiz + psr3*uiz
               turi = -psr5*urk - psr5*urk
               turk = -psr5*uri - psr5*uri
...
c
               txi5 = 2.0d0 * (psr5*ukx+psr5*ukx)
               tyi5 = 2.0d0 * (psr5*uky+psr5*uky)
               tzi5 = 2.0d0 * (psr5*ukz+psr5*ukz)
               txk5 = 2.0d0 * (psr5*uix+psr5*uix)
               tyk5 = 2.0d0 * (psr5*uiy+psr5*uiy)
               tzk5 = 2.0d0 * (psr5*uiz+psr5*uiz)
               turi = -psr7*urk - psr7*urk
               turk = -psr7*uri - psr7*uri
...
!$OMP& x,y,z,xaxis,yaxis,zaxis,rpole,uind,n12,i12,n13,i13,n14,
...
c
      if (boundary .eq. 'VACUUM') then
         ...
         do i = 1, npole
            ii = ipole(i)
            dep(1,ii) = dep(1,ii) + term*rpole(1,i)*(xu+xu)
            dep(2,ii) = dep(2,ii) + term*rpole(1,i)*(yu+yu)
            dep(3,ii) = dep(3,ii) + term*rpole(1,i)*(zu+zu)
         end do
         xufield = -term * (xu+xu)
         yufield = -term * (yu+yu)
         zufield = -term * (zu+zu)
...
c
         ...
         yv = yq * (yu+yu)
         zv = zq * (zu+zu)
         vterm = xv + yv + zv + xu*xu + yu*yu + zu*zu
     &              + xd*(xu+xu) + yd*(yu+yu) + zd*(zu+zu)
         vterm = term * vterm
...
c
      ...
      do i = 1, npole
         ...
         do j = 1, 20
            ...
            !Here we found the "2-time"
            !write(*,*) "fphidp", fphidp(j,i) 
         end do
      end do
c
...
c
      ...
      do i = 1, npole
         ...
         do k = 1, 3
            ...
            f1 = f1 + (fuind(k,i)+fuind(k,i))*fphi(j1,i)
     &              + fuind(k,i)*fphid(j1,i)
     &              + fuind(k,i)*fphid(j1,i)
            f2 = f2 + (fuind(k,i)+fuind(k,i))*fphi(j2,i)
     &              + fuind(k,i)*fphid(j2,i)
     &              + fuind(k,i)*fphid(j2,i)
            f3 = f3 + (fuind(k,i)+fuind(k,i))*fphi(j3,i)
     &              + fuind(k,i)*fphid(j3,i)
     &              + fuind(k,i)*fphid(j3,i)
            if (poltyp.eq.'DIRECT' .or. poltyp.eq.'OPT') then
               f1 = f1 - fuind(k,i)*fphid(j1,i)
     &                 - fuind(k,i)*fphid(j1,i)
               f2 = f2 - fuind(k,i)*fphid(j2,i)
     &                 - fuind(k,i)*fphid(j2,i)
               f3 = f3 - fuind(k,i)*fphid(j3,i)
     &                 - fuind(k,i)*fphid(j3,i)
            end if
         end do
         do k = 1, 10
...
c
     ...
     &             - 0.5d0*(cphim(2)*(uind(1,i)+uind(1,i))
     &                     +cphid(2)*uind(1,i)+cphid(2)*uind(1,i))
         vxy = vxy - 0.5d0*(cphi(2,i)*cmp(3,i)+cphi(3,i)*cmp(2,i))
     &             - 0.25d0*(cphim(2)*(uind(2,i)+uind(2,i))
     &                      +cphim(3)*(uind(1,i)+uind(1,i))
     &                      +cphid(2)*uind(2,i)+cphid(2)*uind(2,i)
     &                      +cphid(3)*uind(1,i)+cphid(3)*uind(1,i))
         vxz = vxz - 0.5d0*(cphi(2,i)*cmp(4,i)+cphi(4,i)*cmp(2,i))
     &             - 0.25d0*(cphim(2)*(uind(3,i)+uind(3,i))
     &                      +cphim(4)*(uind(1,i)+uind(1,i))
     &                      +cphid(2)*uind(3,i)+cphid(2)*uind(3,i)
     &                      +cphid(4)*uind(1,i)+cphid(4)*uind(1,i))
         vyy = vyy - cphi(3,i)*cmp(3,i)
     &             - 0.5d0*(cphim(3)*(uind(2,i)+uind(2,i))
     &                     +cphid(3)*uind(2,i)+cphid(3)*uind(2,i))
         vyz = vyz - 0.5d0*(cphi(3,i)*cmp(4,i)+cphi(4,i)*cmp(3,i))
     &             - 0.25d0*(cphim(3)*(uind(3,i)+uind(3,i))
     &                      +cphim(4)*(uind(2,i)+uind(2,i))
     &                      +cphid(3)*uind(3,i)+cphid(3)*uind(3,i)
     &                      +cphid(4)*uind(2,i)+cphid(4)*uind(2,i))
         vzz = vzz - cphi(4,i)*cmp(4,i)
     &             - 0.5d0*(cphim(4)*(uind(3,i)+uind(3,i))
     &                     +cphid(4)*uind(3,i)+cphid(4)*uind(3,i))
     ...
     &             - cmp(10,i)*cphi(10,i)
         if (poltyp.eq.'DIRECT' .or. poltyp.eq.'OPT') then
            vxx = vxx + 0.5d0*(cphid(2)*uind(1,i)+cphid(2)*uind(1,i))
            vxy = vxy + 0.25d0*(cphid(2)*uind(2,i)+cphid(2)*uind(2,i)
     &                        +cphid(3)*uind(1,i)+cphid(3)*uind(1,i))
            vxz = vxz + 0.25d0*(cphid(2)*uind(3,i)+cphid(2)*uind(3,i)
     &                        +cphid(4)*uind(1,i)+cphid(4)*uind(1,i))
            vyy = vyy + 0.5d0*(cphid(3)*uind(2,i)+cphid(3)*uind(2,i))
            vyz = vyz + 0.25d0*(cphid(3)*uind(3,i)+cphid(3)*uind(3,i)
     &                        +cphid(4)*uind(2,i)+cphid(4)*uind(2,i))
            vzz = vzz + 0.5d0*(cphid(4)*uind(3,i)+cphid(4)*uind(3,i))
         end if
      end do
c
...
c
     ...
     &                               + a(j,3)*uopt(m,3,i)
                  ...
                  do j = 1, 3
                     ...
                     j2 = deriv2(j+1)
                     j3 = deriv3(j+1)
                     f1 = f1 + fuind(j,i)*fphid(j1,i)
     &                       + fuind(j,i)*fphid(j1,i)
                     f2 = f2 + fuind(j,i)*fphid(j2,i)
     &                       + fuind(j,i)*fphid(j2,i)
                     f3 = f3 + fuind(j,i)*fphid(j3,i)
     &                       + fuind(j,i)*fphid(j3,i)
                  end do
                  f1 = 0.5d0 * dble(nfft1) * f1
                  f2 = 0.5d0 * dble(nfft2) * f2
                  f3 = 0.5d0 * dble(nfft3) * f3
                  ...
                  vxx = vxx - 0.5d0*copm(k+m+1)*(cphid(2)*uopt(m,1,i)
     &                                          +cphid(2)*uopt(m,1,i))
                  vxy = vxy - 0.25d0*copm(k+m+1)*(cphid(2)*uopt(m,2,i)
     &                                           +cphid(2)*uopt(m,2,i)
     &                                           +cphid(3)*uopt(m,1,i)
     &                                           +cphid(3)*uopt(m,1,i))
                  vxz = vxz - 0.25d0*copm(k+m+1)*(cphid(2)*uopt(m,3,i)
     &                                           +cphid(2)*uopt(m,3,i)
     &                                           +cphid(4)*uopt(m,1,i)
     &                                           +cphid(4)*uopt(m,1,i))
                  vyy = vyy - 0.5d0*copm(k+m+1)*(cphid(3)*uopt(m,2,i)
     &                                          +cphid(3)*uopt(m,2,i))
                  vyz = vyz - 0.25d0*copm(k+m+1)*(cphid(3)*uopt(m,3,i)
     &                                           +cphid(3)*uopt(m,3,i)
     &                                           +cphid(4)*uopt(m,2,i)
     &                                           +cphid(4)*uopt(m,2,i))
                  vzz = vzz - 0.5d0*copm(k+m+1)*(cphid(4)*uopt(m,3,i)
     &                                          +cphid(4)*uopt(m,3,i))
               end do
            end do
         end do
...
c
      ...
      do i = 1, npole
         do j = 2, 4
            cmp(j,i) = cmp(j,i) + uind(j-1,i) - uind(j-1,i)
         end do
      end do
      call cmp_to_fmp (cmp,fmp)
      call grid_mpole (fmp)
