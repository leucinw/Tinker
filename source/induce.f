...
c
     ...
     &                          + field(j,i)
         ...
         mode = 'BUILD'
         if (use_mlist) then
            call uscale0b (mode,rsd,zrsd)
            mode = 'APPLY'
            call uscale0b (mode,rsd,zrsd)
         else
            call uscale0a (mode,rsd,zrsd)
            mode = 'APPLY'
            call uscale0a (mode,rsd,zrsd)
         end if
         do i = 1, npole
...
c
     ...
     &                        - rr3*diz - 2.0d0*rr5*qiz
                  do j = 1, 3
                     field(j,i) = field(j,i) + fid(j)*pscale(kk)
                     field(j,k) = field(j,k) + fkd(j)*pscale(kk)
                  end do
               end if
            end if
         end do
...
c
     ...
     &                        - rr3*diz - 2.0d0*rr5*qiz
                     if (use_polymer .and. r2.le.polycut2) then
                        do j = 1, 3
                           fid(j) = fid(j) * pscale(kk)
                           fkd(j) = fkd(j) * pscale(kk)
                        end do
                     end if
                     do j = 1, 3
...
c
      if (use_mlist) then
         call udirect2b (field)
      else
         call udirect2a (field)
      end if
c
...
c
      if (use_mlist) then
         call umutual2b (field)
      else
         call umutual2a (field)
      end if
c
c     get the self-energy portion of the electrostatic field
c
...
c
     ...
     &                            + (9.0d0/60.0d0)*damp**2.0d0)*expdamp
                     ...
                     if (use_polymer) then
                        if (r2 .le. polycut2) then
                           dsc3 = scale3 * pscale(kk)
                           dsc5 = scale5 * pscale(kk)
                           dsc7 = scale7 * pscale(kk)
                        end if
                     end if
c
c     find the field terms for the current interaction
...
!$OMP& nthread,nchunk)
!$OMP& firstprivate(pscale,uscale,nlocal)
c
c     perform dynamic allocation of some local arrays
c
...
c     compute the real space portion of the Ewald summation
c
!$OMP DO reduction(+:fieldt) schedule(static,nchunk)
      do i = 1, npole
         ii = ipole(i)
         pdi = pdamp(i)
         pti = thole(i)
...
c
      call fftback
      call fphi_uind (fdip_phi1,fdip_sum_phi)
c
...
c     OpenMP directives for the major loop structure
c
!$OMP PARALLEL default(private) shared(npole,uind,ntpair,tindex,
!$OMP& tdipdip,field,fieldt)
!$OMP DO reduction(+:fieldt) schedule(guided)
c
...
c
c
      subroutine uscale0a (mode,rsd,zrsd)
      use sizes
      use atoms
      use limits
      use mpole
...
c
c
      subroutine uscale0b (mode,rsd,zrsd)
      use sizes
      use atoms
      use mpole
      use neigh
...
!$OMP& rsd,zrsd,zrsdt)
!$OMP DO reduction(+:zrsdt) schedule(guided)
         do i = 1, npole
            m = mindex(i)
            do kk = 1, nulst(i)
               k = ulst(kk,i)
